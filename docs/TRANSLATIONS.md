...
Navigate to the ***Language*** folder (Located inside of `source\FluentAurora\Resources\Language`) and find the appropriate `.axaml` file:
...
2. **Understanding the file structure**
   ```xml
   <sys:String x:Key="App.Title">FluentAurora</sys:String>
   ```
...
1. **Enable your language in the code**
   - Find the `SupportedLanguages` array in the code (found in `source\FluentAurora.Core\Utilities\LocalizationHelper.cs`)
...
### Option 1: Using Git
```bash
git add source/FluentAurora/Resources/Language/CODE.axaml
